...
import type { ITeamRepository } from "@/core/repositories/team.repository";
...
const fetchTeamStats = cache(async (teamId: number): Promise<TeamStats> => {
  const data = await betlabFetch<ApiTeamStatsResponse>(`/api/teams/${teamId}/stats`);
  return transformTeamStats(data.stats);
});

export class BetlabTeamRepository implements ITeamRepository {
